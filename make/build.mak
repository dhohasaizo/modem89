...
ifneq ($(strip $(NO_PCIBT)),TRUE)
 ...
 -include make/folderlist.bld
     ifneq ($(words $(FOLDERLIST)),0)
       FOLDERLIST := $(subst \,/,$(FOLDERLIST))
       TEMP1 := $(filter-out $(FOLDERLIST),$(TEMP4))
       TEMP2 := $(filter-out custom/% $(subst \,/,$(strip $(BUILDDIR))/%) build/%,$(TEMP1))
       ifneq ($(strip $(TEMP2)),)
         PERMISSON_ERROR_PATH := $(foreach DIR,$(TEMP2),$(wildcard $(DIR)))
         NOT_EXISTED_PATH := $(filter-out $(PERMISSON_ERROR_PATH),$(TEMP2))
         ...
         ifneq ($(strip $(NOT_EXISTED_PATH)),)
           $(shell perl -e "print \"Error: The following global include paths do not exist in perforce: '$(NOT_EXISTED_PATH)' \n\";">> $(strip $(TARGDIR))/GlobalIncCheck.log)
...
ifeq ($(strip $(MBIS_EN)),TRUE)
 ...
 ifeq ($(strip $(PARTIAL_SOURCE)),TRUE)
		$(strip $(LINKER)) -gcc TARGET $(strip $(LINK)) $(strip $(LOG)) $(strip $(VIA)) $(strip $(BIN_CREATE)) $(strip $(TARGDIR)) $(strip $(IMG_FILE)) "$(strip $(BIN_FORMAT))" $(strip $(BIN_FILE)) $(strip $(BUILD_TST_DB)) $(TARGNAME).map $(TARGNAME).sym $(strip $(TMPDIR)) "$(strip $(CPL))" $(strip $(DUMP))
  else
		@$(strip $(LINK)) $(strip $(VIA))$(strip $(TMPDIR))/~libs.tmp > $(TARGDIR)/link.log 2>&1
		@$(IF_EXIST) $(strip $(TARGDIR))/$(TARGNAME).elf $(THEN)\
			$(strip $(DUMP)) -x $(strip $(TARGDIR))/$(TARGNAME).elf > $(strip $(TARGDIR))/$(TARGNAME).sym 2>&1 \
		$(CMD_END)
  endif
...
# *************************************************************************
# TARGET: LINK_BIN_FILE_CHECK POSTBUILD_CHECK
#  Executable Targets
# *************************************************************************
...
ifneq ($(strip $(COMPILER)),GCC)
  # mbis time probe
	...
	@perl tools\auto_adjust_mem.pl 1 $(LOG) $(strip $(COMPLOGDIR))\ckImgSize.log "$(strip $(PROJECT_MAKEFILE))" $(strip $(COMPLOGDIR))\ckSysDrv.log $(strip $(COMPLOGDIR))\CheckBinaryBlockUsage.log $(FIXPATH)\custom\system\$(strip $(BOARD_VER)) ~flash_cfg_tmp.c ~fs_quota_usage.log ~nvram_lid_size.log $(strip $(FEATURE_OVERLOAD)) $(strip $(SYSTEM_DRIVE_ON_NAND)) $(strip $(NVRAM_PSEUDO_MERGE)) $(strip $(PROJECT_NAME))>> $(strip $(COMPLOGDIR))\auto_adjust_mem.log & \
		(if ERRORLEVEL 1 \
		(tools\make.exe -fmake\build.mak -r -R update_memconfig) & \
...
ifeq ($(strip $(MBIS_EN)),TRUE)
	...
	@$(IF_NOT_EXIST) $(strip $(TARGDIR))/$(IMG_FILE) $(THEN) (echo $(strip $(TARGDIR))/$(IMG_FILE) is not existed!Please check link error. $(WITH) exit 1) $(CMD_END)
...
# Binding the binary for anticlone.
	...
	@$(IF_EXIST) $(strip $(TARGDIR))/$(strip $(BIN_FILE)) $(THEN)\
		(perl tools/append2.pl $(strip $(FLASHFILE)) $(strip $(TARGDIR))/$(strip $(BIN_FILE)) $(call Upper,$(strip $(PROJECT_NAME))_$(strip $(SUB_BOARD_VER))_$(strip $(PLATFORM))_$(strip $(CHIP_VER))).$(strip $(PURE_VERNO)).bin $(VERNO) "$(strip $(PROJECT_MAKEFILE))" "$(strip $(BUILD_DATE_TIME))" $(strip $(TMPDIR)) > $(strip $(COMPLOGDIR))/append2.log 2>&1 $(WITH) \
...
ifneq ($(strip $(NAND_FLASH_BOOTING)),NONE)
	@$(IF_EXIST) ./tools/CheckBinaryBlockUsage.pl $(THEN)\
		(perl ./tools/CheckBinaryBlockUsage.pl $(strip $(TMPDIR))/~flash_cfg_tmp.c $(strip $(TARGDIR))/$(strip $(BIN_FILE)) $(strip $(NEED_BUILD_BOOTLOADER)) $(strip $(TARGDIR))/$(strip $(BTLD_BIN_FILE)) $(strip $(TARGDIR))/$(strip $(BTLD_EXT_BIN_FILE)) $(strip $(PLATFORM))> $(strip $(COMPLOGDIR))/CheckBinaryBlockUsage.log 2>&1) $(WITH) \
...
ifneq ($(filter $(strip $(PLATFORM)),$(8_BIT_ENCRYPTION_PLATFORM)),)
	...
	@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE))\ROM  \
		(perl tools\append2.pl $(strip $(FLASHFILE)) $(strip $(TARGDIR))\$(strip $(BIN_FILE))\ROM $(call Upper,$(strip $(PROJECT_NAME))_$(strip $(SUB_BOARD_VER))_$(strip $(PLATFORM))_$(strip $(CHIP_VER))).$(strip $(PURE_VERNO)).bin $(VERNO) "$(strip $(PROJECT_MAKEFILE))" "$(strip $(BUILD_DATE_TIME))" $(strip $(TMPDIR)) $(strip $(SCATTERFILE)) > $(strip $(COMPLOGDIR))\append2.log 2>&1) & \
...
ifneq ($(filter $(strip $(PLATFORM)),$(8_BIT_ENCRYPTION_PLATFORM)),)
	...
	(if ERRORLEVEL 1 \
		(echo Error: Failed in update_img.exe) & \
		(exit 1)) & \
	(perl tools\append2.pl $(strip $(FLASHFILE)) $(strip $(TARGDIR))\$(strip $(BIN_FILE)) $(call Upper,$(strip $(PROJECT_NAME))_$(strip $(SUB_BOARD_VER))_$(strip $(PLATFORM))_$(strip $(CHIP_VER))).$(strip $(PURE_VERNO)).bin $(VERNO) "$(strip $(PROJECT_MAKEFILE))" "$(strip $(BUILD_DATE_TIME))" $(strip $(TMPDIR)) $(strip $(SCATTERFILE))> $(strip $(COMPLOGDIR))\append2.log 2>&1) & \
	(if ERRORLEVEL 1 (echo Error: append2.pl Failed!! Please check $(strip $(COMPLOGDIR))\append2.log))
...
ifneq ($(strip $(NAND_FLASH_BOOTING)),NONE)
	@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE))\ROM  \
		(perl .\tools\CheckBinaryBlockUsage.pl $(TMPDIR)\~flash_cfg_tmp.c $(strip $(TARGDIR))\$(strip $(BIN_FILE)) $(strip $(NEED_BUILD_BOOTLOADER)) $(strip $(TARGDIR))\$(strip $(BTLD_BIN_FILE))  $(strip $(TARGDIR))\$(strip $(BTLD_EXT_BIN_FILE)) > $(strip $(COMPLOGDIR))\CheckBinaryBlockUsage.log 2>&1) & \
...
ifneq ($(strip $(NAND_FLASH_BOOTING)),NONE)
	@if exist $(strip $(TARGDIR))\$(strip $(BIN_FILE))\ROM  \
		(perl .\tools\CheckBinaryBlockUsage.pl $(strip $(TMPDIR))\~flash_cfg_tmp.c $(strip $(TARGDIR))\$(strip $(BIN_FILE)) $(strip $(NEED_BUILD_BOOTLOADER)) $(strip $(TARGDIR))\$(strip $(BTLD_BIN_FILE)) $(strip $(TARGDIR))\$(strip $(BTLD_EXT_BIN_FILE)) $(strip $(PLATFORM)) > $(strip $(COMPLOGDIR))\CheckBinaryBlockUsage.log 2>&1) & \
...
ifeq ($(strip $(MBIS_EN)),TRUE)
   ...
   # can be moved to genlog
	...
	@$(IF_EXIST) $(strip $(TARGDIR))/$(TARGNAME).lis $(THEN) ($(DEL_FILE) $(strip $(TARGDIR))/$(strip $(TARGNAME)).lis) $(CMD_END)
...
ifneq ($(strip $(MODIS_CONFIG)),TRUE)
	...
	$(ELSE) \
		...
		($(IF_EXIST) $(strip $(TARGDIR))/$(TARGNAME).lis $(THEN) ($(DEL_FILE) $(strip $(TARGDIR))/$(TARGNAME).lis) $(CMD_END)) $(AND) \
...
ifeq ($(strip $(MBIS_EN)),TRUE)
   ...
   # -----------------------------
	...
	$(ELSE) \
		...
		($(IF_EXIST) $(strip $(TARGDIR))/$(TARGNAME).lis $(THEN) ($(DEL_FILE) $(strip $(TARGDIR))/$(TARGNAME).lis) $(CMD_END)) $(WITH) \
...
# mbis time probe
  ifeq ($(strip $(MBIS_EN)),TRUE)
	...
	@$(IF_EXIST) $(strip $(TARGDIR))/ZIMAGE_DEBUG           $(THEN) ($(DEL_DIR)  $(strip $(TARGDIR))/ZIMAGE_DEBUG)           $(CMD_END)
	@$(IF_EXIST) $(strip $(TARGDIR))/DEMAND_PAGING_DEBUG    $(THEN) ($(DEL_DIR)  $(strip $(TARGDIR))/DEMAND_PAGING_DEBUG)    $(CMD_END)
	@$(IF_EXIST) $(strip $(TARGDIR))/FILE_SYSTEM_DEBUG      $(THEN) ($(DEL_DIR)  $(strip $(TARGDIR))/FILE_SYSTEM_DEBUG)      $(CMD_END)
# mbis time probe
  ifeq ($(strip $(MBIS_EN)),TRUE)
	@$(ECHO) -n -t "T_E,$@,T," >> $(MBIS_BUILD_TIME_TMP)
  endif
