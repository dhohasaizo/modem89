...
/***************************************************************************
*  PRIMITIVE STRUCTURE
*     mphc_power_scan_stop_req_struct
*
*  DESCRIPTION
*     This message is sent from L1A to the upper layer for stopping the
*     power scan procedure. It contains the power scan stop tid
*
***************************************************************************/
typedef struct
{
...
/*****************************************************************************
* PRIMITIVE STRUCTURE
*   MPHP_MONITOR_PAGE_IN_PTM_START_REQ
* DESCRIPTION
*   Message request to ask layer 1 to start pch monitoring.
*   It is sent to L1.
*****************************************************************************/
typedef struct
{
   ...
   kal_int32   bs_pa_mfrms;               /* The number of 51-multiframes between
                                             transmission of paging messages to
                                             mobiles of the same paging group, 2~9 */
   kal_int32   smart_bs_pa_mfrms;         /* The number of 51-multiframes between transmission 
                                           * of smart paging. Could be 1/2/3/4 times of bs_pa_mfrms */
   kal_int32   bs_ag_blks_res;            /* The number of blocks on each common
                                             control channel reserved for access
                                             grant messages, 0~7 */
   kal_int32   bs_cc_chans;               /* The number of common control channel, 1~4 */
   kal_bool    bs_ccch_sdcch_comb;        /* It indicates if CCCh and SDCCh are
                                             combined into one channel */
   kal_uint8   smart_mfrms_group_offset;  /* smart_bs_pa_mfrms / bs_pa_mfrms = 1,2,3,4 (PG2~PG9) *
                                           * so group offset = 0 ,1 ,2, 3 to indicate the index  *
                                           * of group in smart_bs_pa_mfrms. ex. index = 1, means *
                                           * the sec. bs_pa_mfrms in the smart_ba_pa_mfrms       */
   mph_paging_mode_enum   paging_mode;    /* The indication of paging mode,*
                                           * MPH_NORMAL_PAGING,            *
                                           * MPH_EXTENDED_PAGING,          *
                                           * MPH_PAGING_REORGANIZATION,    *
                                           * MPH_SAME_AS_BEFORE            */
}mphp_monitor_page_in_ptm_start_req_struct;

/*****************************************************************************
* PRIMITIVE STRUCTURE
*   MPHP_MONITOR_PAGE_IN_PTM_STOP_REQ
* DESCRIPTION
*   Message request to ask layer 1 to stop pch monitoring.
*   It is sent to L1.
*****************************************************************************/
typedef struct
{
   kal_uint8   ref_count;
   kal_uint16  msg_len;
}mphp_monitor_page_in_ptm_stop_req_struct;
#endif /* end of __MONITOR_PAGE_DURING_TRANSFER__ */

#if defined(__SMART_PAGING_IN_IDLE__) || defined(__MONITOR_PAGE_DURING_TRANSFER__)
/***************************************************************************
*  PRIMITIVE STRUCTURE
*     mphc_smart_page_start_req_struct
*
*  DESCRIPTION
*     This message is sent from the upper layer to L1A for request to do smart paging. 
*
***************************************************************************/
typedef struct
{
...
#endif /*(__SMART_PAGING_IN_IDLE__) || (__MONITOR_PAGE_DURING_TRANSFER__)*/
...
typedef struct
{
   ...
   /* serving cell power of ReportPCh for Smart Paging*/
   ...
   mph_block_monitor_enum     rx_block_status;//Pop fix ReportPChSharing
...
typedef struct
{
   ...
   kal_uint8 mr_ver;          /* MultiRate speech VERsion : 001-amr verion 1, 002- amr version 2 wb_amr*/
...
#ifdef __GERAN_R6__ 
   /* use the pwr level from MPAL's decision for immediate assignment*/
   kal_uint8                         pwr_level;
#endif  /*__GERAN_R6__ */
#ifdef __VAMOS_2__
   kal_uint8                         vamos_level; //0(non-vamos), 1(vamos1), 2(vamos2)
#endif /*__VAMOS_2__*/
} mphc_immed_assign_req_struct;
...
typedef struct
{
   ...
   kal_int8                     gsm900_power_class_8psk;  /* MPH_POWER_CLASS_1 = 0,
                                                     MPH_POWER_CLASS_2 = 1,
                                                     MPH_POWER_CLASS_3 = 2,
                                                     MPH_POWER_CLASS_4 = 3,
                                                     MPH_POWER_CLASS_5 = 4,
...
typedef struct
{
   ...
   /* It indicates if it is received on extended channel */
   kal_bool                   is_extended;
   mph_block_monitor_enum     rx_block_status;
   kal_int8                   tc;
#ifdef __PS_SERVICE__
   /* C-value calculated by L1 for GPRS case */
   kal_int16                  c_value;
...
typedef struct
{
   kal_uint8   ref_count;
   kal_uint16  msg_len;
   /* The power measurement results in uint of 1/4dbm */
   Power       rlac_in_quarter_dbm;
} mphc_serv_idle_meas_ind_struct;
...
typedef struct
{
   ...
   kal_bool    dtx_used;
   kal_int16   current_tx_power_in_dbm;  /* Shihyao20100422, add this parameter to show the transmitted power level in engineering mode */
   ...
   kal_bool   dl_dtx;  /* Shihyao20100422, add this parameter to show if dl dtx is detected in engineering mode */
...
typedef struct
{
   ...
   kal_uint8   list_len;   /* The number of valid elements of rlac_in_quarter_dbm */
   /* The list arfcn on which the measurements are taken */
   ARFCN       arfcn_list[MPH_MAX_BA_LIST_LEN+1];                                                 
   /* The measurement average for each carrier in unit of 1/4dbm */
...
typedef struct
{
   ...
   /* Elly071217, for handling race condition */
   kal_uint8                              bsic_tid;
...
typedef struct
{
  ...
  mphc_open_tch_loop_req_struct,
  mphc_open_tch_loop_cnf_struct,
  mphc_cell_selection_init_req_struct,
  mphc_cell_selection_init_cnf_struct,
#ifdef __GEMINI_GSM__
  rsvas_l1_suspend_req_struct,
  rsvas_l1_suspend_cnf_struct,
  rsvas_l1_resume_req_struct,
#ifdef __GEMINI_MONITOR_PAGE_DURING_TRANSFER__
  /* Shihyao, gemini 2.0 development */
  mphc_virtual_suspend_req_struct,
  mphc_virtual_suspend_cnf_struct,
  mphc_resume_req_struct,
  rsvas_l1_virtual_resume_req_struct,
#endif /* end of __GEMINI_MONITOR_PAGE_DURING_TRANSFER__ */
#endif /* end of __GEMINI_GSM__ */
#if defined (L1A_SIM) && !defined (__GEMINI_GSM__) /* To run WG_0, GW_0 scripts in Dual_Mode environment*/
  rsvas_l1_suspend_req_struct,
  rsvas_l1_suspend_cnf_struct,
  mphc_virtual_suspend_req_struct,
  mphc_virtual_suspend_cnf_struct,
#endif /* end of (__L1A_SIM__) && !(__GEMINI_GSM__)*/   
  mphc_deactivate_req_struct;
...
*   MPHC_ACTIVE_REQ
* DESCRIPTION
*   Message requesting layer 1 to enter active RAT.
*****************************************************************************/
typedef struct
{
  kal_uint8         ref_count;
...
} mphc_active_cnf_struct;

#ifdef __BLIND_HANDOVER__
/***************************************************************************
*  PRIMITIVE STRUCTURE
*     mphc_blind_handover_bsic_req_struct
*
*  DESCRIPTION
*     This message is sent from the upper layer to L1A for requesting L1
*     to decode BSIC on the specified carriers as blind handover. L1 will report any BSIC
*     immediately after decoding it.
...
/***************************************************************************
*  PRIMITIVE STRUCTURE
*     mphc_blind_handover_bsic_ind_struct
*
*  DESCRIPTION
*     This message is sent from L1A to the upper layer for the results
*     of BSIC decoding as blind handover. 
*
***************************************************************************/
typedef struct
{
...
typedef struct
{
   kal_uint8     ref_count;
   kal_uint16    msg_len;
   kal_uint8     sim_config_status;  // bitmap from right to left : 0x01 for SIM1 ; 0x02 for SIM2, 0x04 for SIM3 ... etc.
   kal_uint8     sim_insert_status;  // bitmap from right to left : 0x01 for SIM1 ; 0x02 for SIM2, 0x04 for SIM3 ... etc.
} mphc_sim_status_update_req_struct;

#ifdef __UMTS_RAT__
/*****************************************************************************
* STRUCT
*   mphc_alt_l1_gsm_gap_service_update_req_struct
*
* DESCRIPTIONS
*    Used for the primitive MPHC_ALT_L1_GSM_GAP_GIVEN_IND. This primitive is used
*    to inform alt L1 that this L1 has been given gsm gap pattern to UMTS
*
* PARAMETERS
*   ref_count   
*   msg_len    
...
/*****************************************************************************
* STRUCT
*   mphc_alt_l1_gsm_gap_resume_req_struct
*
* DESCRIPTIONS
*    Used for the primitive MPHC_ALT_L1_GSM_GAP_STOP_CNF. This primitive is used for
*    to inform alt L1 that this L1 has been cancelled gsm gap pattern to UMTS
*
* PARAMETERS
*   ref_count   
*   msg_len    
...
/*****************************************************************************
* STRUCT
*   mphc_channel_protect_start_req_struct
*
* DESCRIPTIONS
*    Used for the primitive MPHC_CHANNEL_PROTECT_START_REQ. This primitive is used for
*    to inform alt L1 that these channels should be protected.
*
* PARAMETERS
*   ref_count   
*   msg_len    
...
typedef struct
{
  kal_uint8        ref_count;
  kal_uint16       msg_len;
  kal_uint32       channel_protected;
} mphc_channel_protect_start_req_struct;

/*****************************************************************************
* STRUCT
*   mphc_channel_protect_stop_req_struct
*
* DESCRIPTIONS
*    Used for the primitive MPHC_CHANNEL_PROTECT_STOP_REQ. This primitive is used for
*    to inform alt L1 that the protection of these channels shuold be cancelled.
*
* PARAMETERS
*   ref_count   
*   msg_len    
 ...
 *  then L1C calls L1A's function to return primitive to L4C */

/***************************************************************************
*  PRIMITIVE STRUCTURE
*     mphc_em_rf_test_gsm_tx_test_struct
*
*  DESCRIPTION
*     This message is sent from L4C to L1A to set L1C's parameter
*     for RF tool usage in engineering mode
***************************************************************************/
typedef struct
{
   kal_uint8         ref_count;
...
} mphc_em_rf_test_gsm_tx_test_req_struct;
/***************************************************************************
*  PRIMITIVE STRUCTURE
*     mphc_em_rf_test_gsm_rx_test_struct
*
*  DESCRIPTION
*     This message is sent from L4C to L1A to set L1C's parameter
*     for RF tool usage in engineering mode
...
typedef struct
{
   ...
   kal_uint8 band;       /* input */
   kal_uint8  pattern;   /* input */
} mphc_em_rf_test_gsm_rx_test_req_struct;
/***************************************************************************
*  PRIMITIVE STRUCTURE
*     mphc_em_rf_test_gsm_power_scan_struct
*
*  DESCRIPTION
*     This message is sent from L4C to L1A to set L1C's parameter
*     for RF tool usage in engineering mode
...
/***************************************************************************
*  PRIMITIVE STRUCTURE
*     l1_gsm_max_tx_pwr_red_req_struct
*
*  DESCRIPTION
*     This message is sent to L1 to set the reduction of maximum 
*     TX power in a 1/8 dB step, the available range is 0~64(0~8dB),
*     and any out of range value(65~255) would be treated as 64(8dB)
***************************************************************************/
typedef struct
{
   kal_uint8         ref_count;
   ...
   kal_bool          valid;
   kal_uint8         op;
   kal_uint8         Gsm_tx_reduction[4][2][4];   //  Band, modulation, TX slot cnt
} l1_gsm_max_tx_pwr_red_req_struct;
...
#ifndef __L1_STANDALONE__
/*  The two messages are added for checking calibration data
 *  downlink status, L4C would send 1 request message to L1, and L1
 *  would return 1 message to L4C in response */
/***************************************************************************
*  PRIMITIVE STRUCTURE
*     l4cl1_query_cal_data_dl_status_req_struct
*
*  DESCRIPTION
*     This message is sent from L4C to L1A to request the calibration
*     data downlink status. src_id is used for l4 to know which is this 
*     request from, and l4 will send confirm back according to src_id copied
*     in cnf.
***************************************************************************/
typedef struct
{
...
/***************************************************************************
*  PRIMITIVE STRUCTURE
*     l4cl1_query_cal_data_dl_status_cnf_struct
*
*  DESCRIPTION
*     This message is sent from L1A to L4C to send the calibration
*     dara downlink status, when cal_data_valid is true, it means 
*     the existance of calibration data. src_id is copied from req,
*     and is used to inform l4 where to send the confirm.
***************************************************************************/
typedef struct
{
   kal_uint8        ref_count;
...
*****************************************************************************/
extern void mpal_facch_downlink(mph_dl_callback_struct mph_dl_callback
                              #ifdef __GEMINI_GSM__
...
*   mode. This function is called by L1, and provided by MPAL.
*****************************************************************************/
extern void mpal_sdcch_downlink(mph_dl_callback_struct mph_dl_callback
                              #ifdef __GEMINI_GSM__
                                , module_id_enum mod_indx
                              #endif
                                );
...
*   mode. This function is called by L1, and provided by MPAL.
*****************************************************************************/
extern void mpal_sacch_downlink(mph_dl_callback_struct mph_dl_callback
                             #ifdef __GEMINI_GSM__
                               , module_id_enum mod_indx
                             #endif
                                );
...
/*****************************************************************************
* CALLBACK FUNCTION PROTOTYPE
*   mpal_detect_paging_block_ind_hdlr
* DESCRIPTION
*   Function call to decode the MS identity in the paging block and check if 
*   paging UE. This function is called by L1, and provided by MPAL.
*****************************************************************************/
extern kal_bool mpal_detect_paging_block_ind_hdlr(kal_uint8 *pch_data
                                             #ifdef __GEMINI_GSM__
                                                , module_id_enum mod_indx
                                             #endif
                                                 );
