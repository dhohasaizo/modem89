...
#    $ENV{"NUMBER_OF_PROCESSORS"} = 1;
    ...
    if ($action !~ /ckmake|check|notify|patch|patch_hal|modis|uesim|sysgen|sys_auto_gen|emigen|nvram_auto_gen|find|rel|cmmgen|removecode|ckcr|elfpatch|codegen|gendsp|viewlog|genmoduleinfo|gen_bt_switch_info|viti/ig) {
      if (($ENV{"SESSIONNAME"} ne "") && ($ENV{"LSF_BINDIR"} ne "") && ($compiler eq "RVCT") && ($ENV{'MTK_INTERNAL'} eq 'TRUE')) {
        $local_path = "";
...
my $AAPMCLOG = "$tmpFolder/AAPMC.log";
unlink($AAPMCLOG) or die "Remove failed $!" if (-e "$AAPMCLOG");
