...
void dcl_pmic6327_internal_init(void)
{
	...
	//Soft step enable after reset
	//bank0 0x57[2:2] = 0x1
		PMIC_DRV_SetData(0x57,0x4,0x4,0x0);
	//Depend on SRCLKEN for sleep mode after reset
	//bank0 0x57[0:0] = 0x1
		PMIC_DRV_SetData(0x57,0x1,0x1,0x0);
	//First set this then turn off and on RG_VMC_EN.  This is so that poorly design memory cards may permanent hang if softstart current limit upon activation (by SA).
	//bank0 0x88[3:3] = 0x1
		PMIC_DRV_SetData(0x88,0x8,0x8,0x0);
	//First disable, then disable STB, then enable
	//bank0 0x88[0:0] = 0x0
