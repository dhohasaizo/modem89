...
TGPD *USB_DMAQ_MBIM_TX_GPD_HEADER;

#ifdef __MTK_TARGET__
__attribute__ ((section ("NONCACHEDZI"), aligned(4))) 
#endif /* __MTK_TARGET__ */
TGPD *USB_DMAQ_MBIM_TX_GPD_QUEUED;

#ifdef __MTK_TARGET__
__attribute__ ((section ("NONCACHEDZI"), aligned(4))) 
#endif /* __MTK_TARGET__ */
TGPD *USB_DMAQ_MBIM_RX_GPD_HEADER;

#ifdef __MTK_TARGET__
__attribute__ ((section ("NONCACHEDZI"), aligned(4))) 
#endif /* __MTK_TARGET__ */
TGPD USB_DMAQ_MBIM_RX_GPD_NULL;

#ifdef __MTK_TARGET__
__attribute__ ((section ("NONCACHEDZI"), aligned(4))) 
#endif /* __MTK_TARGET__ */
TGPD USB_DMAQ_MBIM_TX_GPD_NULL;
#endif
...
//			break;
		...
		case USB_CMD_HALT:
			...
			break;
#endif /*__USB_TETHERING__ || __MBIM_SUPPORT__*/
		...
		case USB_CMD_RX_RECV_DATA:
			{
				...
				if (USB_DMAQ_MBIM_RX_GPD_HEADER != NULL)	//may UPS just flush GPD
				{
					if(((USB_DMAQ_MBIM_RX_GPD_HEADER->flag & RXGPD_FLAGS_HWO) == 0)&&(USB_DMAQ_MBIM_RX_GPD_HEADER != (&USB_DMAQ_MBIM_RX_GPD_NULL)))
					{
						//set buffer header to caller.
						ptr_rUARTGIORXRecvData->pOut_GPD = (DCL_UINT8 *)USB_DMAQ_MBIM_RX_GPD_HEADER;
						
						//search tails.
						p_GPD = USB_DMAQ_MBIM_RX_GPD_HEADER;
						USB_DMAQ_MBIM_RX_GPD_HEADER = USB_DMAQ_MBIM_RX_GPD_HEADER->pNext;
						p_GPD->pNext = NULL;
					}
					else
					{
						g_UsbACM[USB_PORT[dev]].send_Rxilm = KAL_TRUE;
						usb_drv_trace2(USB2UARTPort[USB_PORT[dev]].ownerid, (kal_uint32)USB_DMAQ_RXGPD_INFO, (kal_uint32)USB_DMAQ_MBIM_RX_GPD_HEADER, (kal_uint32)USB_DMAQ_MBIM_RX_GPD_HEADER->pNext);
					}
				}
		...
		case USB_CMD_TX_RECYCLE_GPD:
			{
				...
				if (USB_DMAQ_MBIM_TX_GPD_HEADER != NULL)	//may UPS just flush GPD
				{
					if(((USB_DMAQ_MBIM_TX_GPD_HEADER->flag & TXGPD_FLAGS_HWO) == 0)&&(USB_DMAQ_MBIM_TX_GPD_HEADER != (&USB_DMAQ_MBIM_TX_GPD_NULL)))
					{
						//set buffer header to caller.
						ptr_rUARTGIOTXRecycleGPD->pOut_GPD = (DCL_UINT8 *)USB_DMAQ_MBIM_TX_GPD_HEADER;
	
						//search tails.
						p_GPD = USB_DMAQ_MBIM_TX_GPD_HEADER;
						USB_DMAQ_MBIM_TX_GPD_HEADER = NULL;
						while(((p_GPD->pNext) != (&USB_DMAQ_MBIM_TX_GPD_NULL)))
						{
							if (p_GPD == NULL)
								ASSERT(0);
							p_GPD = p_GPD->pNext;
					...
					{
						usb_drv_trace2(USB2UARTPort[USB_PORT[dev]].ownerid, (kal_uint32)USB_DMAQ_TXGPD_INFO, (kal_uint32)USB_DMAQ_MBIM_TX_GPD_HEADER, (kal_uint32)USB_DMAQ_MBIM_TX_GPD_HEADER->pNext);
					}
				}
				
				if (USB_DMAQ_MBIM_TX_GPD_QUEUED != NULL)
				{
					USB_DMAQ_MBIM_TX_GPD_HEADER = USB_DMAQ_MBIM_TX_GPD_QUEUED;
					USB_DMAQ_MBIM_TX_GPD_QUEUED = NULL;
					if (USB_DMAQ_MBIM_TX_GPD_HEADER != (&USB_DMAQ_MBIM_TX_GPD_NULL))
					{
						drv_trace2(TRACE_FUNC, (kal_uint32)USB_DMAQ_TXGPD_SETUP, (kal_uint32)p_GPD, (kal_uint32)USB_DMAQ_MBIM_TX_GPD_HEADER);                                                                    
						USB_DMAQ_Setup(g_UsbACM[USB_PORT[dev]].txpipe->byEP, USB_EP_TX_DIR, KAL_TRUE, (kal_uint32)USB_DMAQ_MBIM_TX_GPD_HEADER, (usb_dmaq_callback)USB2UART_MBIMTX_DMAQ_Callback, (usb_dmaq_callback)USB2UART_MBIMTX_DMAQ_Empty_Callback);
					}
				}
		...
		case USB_CMD_TX_FLUSH_GPD:
			{
				...
				if (USB_DMAQ_MBIM_TX_GPD_HEADER == NULL || USB_DMAQ_MBIM_TX_GPD_HEADER == (&USB_DMAQ_MBIM_TX_GPD_NULL))
				{
					USB_DMAQ_MBIM_TX_GPD_HEADER = NULL;
					USB_DMAQ_MBIM_TX_GPD_QUEUED = NULL;
				}
				...
				{
					p_GPD = (TGPD *)USB_DMAQ_MBIM_TX_GPD_HEADER;;
					do{
						if (p_GPD->pNext == (&USB_DMAQ_MBIM_TX_GPD_NULL))
							p_GPD->pNext = NULL;
						p_GPD = p_GPD->pNext;
					}while(p_GPD != NULL);
				}
				ptr_rUARTGIOTXFlushGPD->pOut_GPD = (DCL_UINT8 *)USB_DMAQ_MBIM_TX_GPD_HEADER;
				USB_DMAQ_MBIM_TX_GPD_HEADER = NULL;
				USB_DMAQ_MBIM_TX_GPD_QUEUED = NULL;
		...
		case USB_CMD_RX_FLUSH_GPD:
			{
				...
				if (USB_DMAQ_MBIM_RX_GPD_HEADER == NULL || USB_DMAQ_MBIM_RX_GPD_HEADER == (&USB_DMAQ_MBIM_RX_GPD_NULL))
				{
					USB_DMAQ_MBIM_RX_GPD_HEADER = NULL;
				}
				else
				{
					p_GPD = (TGPD *)USB_DMAQ_MBIM_RX_GPD_HEADER;;
					do{
						if (p_GPD->pNext == (&USB_DMAQ_MBIM_RX_GPD_NULL))
							p_GPD->pNext = NULL;
						p_GPD = p_GPD->pNext;
					}while(p_GPD != NULL);
				}

				ptr_rUARTGIORXFlushGPD->pOut_GPD = (DCL_UINT8 *)USB_DMAQ_MBIM_RX_GPD_HEADER;
				USB_DMAQ_MBIM_RX_GPD_HEADER = NULL;
			}
			break;
...
//	module_type src_mod;
	...
	{	// flow : 1.  UPS: config  2. USB: config_Cnf   3. UPS: open port  		
		...
		USB_Send_Msg_Ext_Queue(MOD_UPS, msgid, port_ptr);
#endif
...
//						}
					...
					USB_DMAQ_Setup(g_UsbACM[usb_port].rxpipe->byEP, USB_EP_RX_DIR, 
					  ...
					  (usb_dmaq_callback)USB2UART_PPPRX_DMAQ_Callback, (usb_dmaq_callback)NULL);
#endif
		...
		else if(transfer_type == CDC_HIGH_SPEED_FRAME_DMAQ)
		{
			...
			USB_DMAQ_MBIM_RX_GPD_NULL.flag = 0;
			USB_DMAQ_MBIM_TX_GPD_NULL.flag = 0;
			USB_DMAQ_MBIM_RX_GPD_NULL.pNext = 0;
			USB_DMAQ_MBIM_TX_GPD_NULL.pNext = 0;
...
//		g_UsbACM[usb_port].setup_dma = KAL_TRUE;
		...
		USB_DMAQ_Setup(g_UsbACM[usb_port].txpipe->byEP, USB_EP_TX_DIR, 
		  ...
		  (usb_dmaq_callback)USB2UART_PPPTX_DMAQ_Callback, (usb_dmaq_callback)NULL);
...
static kal_uint16 USB_Receive_Data(UART_PORT port, kal_uint8 **buffaddr, module_type ownerid, kal_uint16 length,kal_uint8 *status,USB_Requ_type type)//,USB_Transfer_type transfer_type)
{
	...
	else
	{
		...
		else if(g_UsbACM[usb_port].transfer_type == CDC_HIGH_SPEED_FRAME_DMAQ)
		{
			...
			if (USB_DMAQ_MBIM_RX_GPD_HEADER == NULL || USB_DMAQ_MBIM_RX_GPD_HEADER == (&USB_DMAQ_MBIM_RX_GPD_NULL))
			{
				USB_DMAQ_MBIM_RX_GPD_HEADER = (TGPD *)buffaddr;
				p_GPD = (TGPD *)buffaddr;;
				do{
					p_GPD->ZTepFlag = (g_UsbACM[usb_port].rxpipe->byEP); //EP
					if (p_GPD->pNext == NULL)
						p_GPD->pNext = &USB_DMAQ_MBIM_RX_GPD_NULL;
					p_GPD = p_GPD->pNext;
				}while(p_GPD != (&USB_DMAQ_MBIM_RX_GPD_NULL));
				
				drv_trace2(TRACE_FUNC, (kal_uint32)USB_DMAQ_RXGPD_SETUP, (kal_uint32)buffaddr, (kal_uint32)USB_DMAQ_MBIM_RX_GPD_HEADER);
				USB_DMAQ_Setup(g_UsbACM[usb_port].rxpipe->byEP, USB_EP_RX_DIR, KAL_TRUE, (kal_uint32) buffaddr, (usb_dmaq_callback)USB2UART_MBIMRX_DMAQ_Callback, (usb_dmaq_callback)USB2UART_MBIMRX_DMAQ_Empty_Callback);
			}
			else
			{
				p_GPD = (TGPD *)buffaddr;
				...
				drv_trace2(TRACE_FUNC, (kal_uint32)USB_DMAQ_RXGPD_INSERT,	(kal_uint32)USB_DMAQ_MBIM_RX_GPD_HEADER, (kal_uint32)buffaddr);
				USB_DMAQ_Insert(g_UsbACM[usb_port].rxpipe->byEP, USB_EP_RX_DIR, (kal_uint32)USB_DMAQ_MBIM_RX_GPD_HEADER, (kal_uint32) buffaddr);
			}
			return 0;
		}
#endif
...
static kal_uint16 USB_Send_Data(UART_PORT port, kal_uint8 *buffaddr, kal_uint16 length, module_type ownerid)//,USB_Transfer_type transfer_type)
{
	...
	else if (g_UsbACM[usb_port].transfer_type == CDC_HIGH_SPEED_FRAME_DMAQ)
	{
		...
		if (USB_DMAQ_MBIM_TX_GPD_HEADER == NULL || USB_DMAQ_MBIM_TX_GPD_HEADER == (&USB_DMAQ_MBIM_TX_GPD_NULL))
		{
			USB_DMAQ_MBIM_TX_GPD_HEADER = (TGPD *)buffaddr;
			p_GPD = (TGPD *)buffaddr;;
			do{
				if (p_GPD->pNext == NULL)
					p_GPD->pNext = &USB_DMAQ_MBIM_TX_GPD_NULL;
				p_GPD = p_GPD->pNext;
			}while(p_GPD != (&USB_DMAQ_MBIM_TX_GPD_NULL));
			
			drv_trace2(TRACE_FUNC, (kal_uint32)USB_DMAQ_TXGPD_SETUP, (kal_uint32)buffaddr, (kal_uint32)USB_DMAQ_MBIM_TX_GPD_HEADER);
			
			USB_DMAQ_MBIM_TX_GPD_QUEUED = NULL;
			USB_DMAQ_Setup(g_UsbACM[usb_port].txpipe->byEP, USB_EP_TX_DIR, KAL_TRUE, (kal_uint32) buffaddr, (usb_dmaq_callback)USB2UART_MBIMTX_DMAQ_Callback, (usb_dmaq_callback)USB2UART_MBIMTX_DMAQ_Empty_Callback);
		}
		else
		{
			...
			if (USB_DMAQ_MBIM_TX_GPD_QUEUED == NULL)
			{
				USB_DMAQ_MBIM_TX_GPD_QUEUED = (TGPD *)buffaddr;
			}
			else
			{
				p_GPD = (TGPD *)USB_DMAQ_MBIM_TX_GPD_QUEUED;
			...
			drv_trace2(TRACE_FUNC, (kal_uint32)USB_DMAQ_TXGPD_INSERT,	(kal_uint32)USB_DMAQ_MBIM_TX_GPD_HEADER, (kal_uint32)buffaddr);
		}
		return 0;
	}
#endif
