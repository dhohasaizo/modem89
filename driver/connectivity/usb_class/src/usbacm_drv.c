...
extern TGPD USB_DMAQ_MBIM_RX_GPD_NULL;
extern TGPD *USB_DMAQ_MBIM_RX_GPD_HEADER;
extern TGPD USB_DMAQ_MBIM_TX_GPD_NULL;
extern TGPD *USB_DMAQ_MBIM_TX_GPD_HEADER;
...
/* DMA callback function for TX sent out data */
void USB2UART_PPPRX_DMAQ_Callback(kal_uint32 usb_port)
{
...
PRAGMA_BEGIN_COMPILER_OPTIMIZE_TIME
void USB2UART_PPPTX_DMAQ_Callback(kal_uint32 usb_port)
{
	UART_PORT port = USB2UARTPort[usb_port].port_no;
...
void USB2UART_MBIMTX_DMAQ_Callback(kal_uint32 usb_port)
{
	//UART_PORT port = USB2UARTPort[usb_port].port_no;
	////if(g_UsbACM[usb_port].send_Txilm == KAL_TRUE)
	//{
...
void USB2UART_MBIMRX_DMAQ_Callback(kal_uint32 usb_port)
{
	UART_PORT port = USB2UARTPort[usb_port].port_no;
	if(g_UsbACM[usb_port].send_Rxilm == KAL_TRUE)
	{
...
void USB2UART_MBIMRX_DMAQ_Empty_Callback(kal_uint32 usb_port)
{
	TGPD *p_GPD = NULL;
	
	drv_trace4(TRACE_FUNC, (kal_uint32)USB_DMAQ_RXGPD_EMPTY, (kal_uint32)USB_DMAQ_MBIM_RX_GPD_HEADER,0,0,0);
	
	if (USB_DMAQ_MBIM_RX_GPD_HEADER != NULL)
	{
		...
		while((p_GPD != (&USB_DMAQ_MBIM_RX_GPD_NULL)))
		{
			...
			{
				drv_trace2(TRACE_FUNC, (kal_uint32)USB_DMAQ_RXGPD_SETUP, (kal_uint32)p_GPD, (kal_uint32)USB_DMAQ_MBIM_RX_GPD_HEADER);
				USB_DMAQ_Setup(g_UsbACM[usb_port].rxpipe->byEP, USB_EP_RX_DIR, KAL_TRUE, (kal_uint32)p_GPD, (usb_dmaq_callback)USB2UART_MBIMRX_DMAQ_Callback, (usb_dmaq_callback)USB2UART_MBIMRX_DMAQ_Empty_Callback);
				break;
			}
			else
			{
...
void USB2UART_MBIMTX_DMAQ_Empty_Callback(kal_uint32 usb_port)
{
	UART_PORT port = USB2UARTPort[usb_port].port_no;
	
	drv_trace4(TRACE_FUNC, (kal_uint32)USB_DMAQ_TXGPD_EMPTY, (kal_uint32)USB_DMAQ_MBIM_TX_GPD_HEADER,0,0,0);
	USB2UART_Sendilm(port, MSG_ID_UART_READY_TO_WRITE_IND);
}
#endif
...
//		((Usb_Cdc_If_Dscr*)g_UsbACM[usb_port].comm_if_info->ifdscr.classif)->iInterface = USB_Get_String_Number((void *)g_UsbACM_Comm.acm_param->comm_interface_string_3);
	...
	if((USB_Check_Composite_With_COM(USB_Get_Device_Type()) == KAL_TRUE) && ((g_UsbMode.usb_comport_driver == USB_COMPORT_WIN)||(g_UsbMode.usb_comport_driver == USB_COMPORT_LINUX)||(g_UsbMode.usb_comport_driver == USB_COMPORT_MBIM)))
	{
