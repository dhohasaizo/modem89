...
#if defined(MT6573) || defined(MT6575) || defined(MT6577) || defined(MT6280)  || defined(MT6589)
...
static void _mt6589_dcm_init(void);
static int _dcm_hw_setting(kal_uint32 on);
   ...
   */
      ...
      #endif
      {
         ...
         if (duration > 60) {
             ...
             *DCM_MDARM_DCM_EN |= 0x3;
             *DCM_MDARM_CCTL &= 0xFFFFFEFF;
          #elif defined(MT6575) || defined (MT6577)  || defined (MT6589)
          ...
          if(WFI_Handle_Count == 0 )
          {
			  ...
			  ccci_fill_idle_task_state(CCCI_IDLE_TASK_DCM_ENTER, usc);
...
#if defined (__MODEM_CCCI_EXIST__)
			  usc = ust_get_current_time();
			  ccci_fill_idle_task_state(CCCI_IDLE_TASK_DCM_LEAVE, usc);
#endif //#if defined (__MODEM_CCCI_EXIST__)
          }
...
/* under construction !*/
      ...
      else // not dcm (sleep or just cpu standby)
      {
          ...
          #if defined(MT6573) || defined(MT6575) || defined (MT6577) || defined(MT6255) || defined(MT6280) || defined (MT6589)
          /* When DTCM is used by other subsys, ARM can not gate clock. */          
          if(WFI_Handle_Count == 0 )
          {
#if defined (__MODEM_CCCI_EXIST__)
			  volatile kal_uint32 usc, type;
			  usc = ust_get_current_time();
			  type = (KAL_TRUE == OST_ReadyToSlept) ? CCCI_IDLE_TASK_SLEEP_ENTER : CCCI_IDLE_TASK_WFI_ENTER;
			  ccci_fill_idle_task_state(type, usc);
#endif //#if defined (__MODEM_CCCI_EXIST__)
...
#if defined (__MODEM_CCCI_EXIST__)
			  usc = ust_get_current_time();
			  type = (type == CCCI_IDLE_TASK_SLEEP_ENTER) ? CCCI_IDLE_TASK_SLEEP_LEAVE : CCCI_IDLE_TASK_WFI_LEAVE;
			  ccci_fill_idle_task_state(type, usc);
#endif //#if defined (__MODEM_CCCI_EXIST__)

          }
          #endif //#if defined(MT6573) || defined(MT6575) || defined (MT6577) || defined(MT6255) || defined(MT6280) || defined (MT6589)
      }           
