...
static CCCI_UARTP_T ccci_uart_port[MAX_CCCI_UART_PORT] =
{
    ...
    ,{ CCCI_SIM_CHANNEL_TX, CCCI_SIM_ACK_CHANNEL_TX, CCCI_SIM_CHANNEL_RX, CCCI_SIM_ACK_CHANNEL_RX, 0, 0,0 /* write_flag */, MOD_NIL, MOD_NIL }
#endif
};

#if defined(__ICUSB_SUPPORT__)
//- disable send ILM to user for SIM port
static const kal_uint32 ccci_uart_send_to_user_enable[MAX_CCCI_UART_PORT] = { \
...
volatile kal_uint32 ccci_uart_retry_expire_time;    //- debug purpose
...
//#define __CCCI_ICUSB_DEBUG__    //- debug log for ICUSB

#if (defined(__CCCI_ICUSB_DEBUG__) && defined(__ICUSB_SUPPORT__))
#define CCCI_UART_DBG_TRACE(...)        kal_prompt_trace(__VA_ARGS__)
#else
#define CCCI_UART_DBG_TRACE(...)
#endif
...
static kal_int32 ccci_uart_send_message_to_ap_retry(kal_uint32 port, kal_uint32 channel, kal_uint32 retry_us)
{
    ...
    while (1) 
    {
        ...
        if ( CCCI_NO_PHY_CHANNEL == ret )
        {
            ...
            {
                ccci_uart_retry_expire_time = kal_get_systicks();
    ...
    CCCI_UART_DBG_TRACE(MOD_TST, "[CCCI UART][Port %d] TX Packet", buff.reserved);
...
#if defined(__ICUSB_SUPPORT__)
    //- Bypass sending ILM for some task.
    if (KAL_FALSE == ccci_uart_send_to_user_enable[port])
        return;
#endif
...
#if defined(__ICUSB_SUPPORT__)
        //- sim uart use virtual port index. remap it to real port index in dcl_uart.h
        //- uart_port_gps_ccci is the last port that can do 1-to-1 mapping. ports larger than it should remap to actual mapping.
        if (local_para->port > uart_port_gps_ccci)
            local_para->port = uart_port_sim_ccci;   //- should change to dynamica mapping if more virtual port is added.
#endif
...
static void ccci_uart_tx_ack_callback(CCCI_BUFF_T *buff)
{
    ...
    CCCI_UART_DBG_TRACE(MOD_TST, "[CCCI UART][Port %d] TXACK RX", port);
    ...
    {
        CCCI_UART_DBG_TRACE(MOD_TST, "[CCCI UART][Port %d] TXACK - Set Event", port);
...
static void ccci_uart_rx_callback(CCCI_BUFF_T *buff)
{
    ...
    CCCI_UART_DBG_TRACE(MOD_TST, "[CCCI UART][Port %d] RX PKT", i);
...
static kal_uint16 ccci_uart_getbytes(DCL_DEV port, kal_uint8 *Buffaddr, kal_uint16 Length, kal_uint8 *status, module_type ownerid)
{
    ...
    CCCI_UART_DBG_TRACE(MOD_TST, "[CCCI UART][Port %d] Read Request, length: %d", port, Length);
    ...
    if (avail != 0)
    {
        ...
        {
            cnt = Length;
            CCCI_UART_DBG_TRACE(MOD_TST, "[CCCI UART][Port %d] Read all, length: %d", port, Length);
        ...
        else // can not output all
        {
            ...
            cnt = avail;
            CCCI_UART_DBG_TRACE(MOD_TST, "[CCCI UART][Port %d] Read partial, length: %d", port, avail);
...
{
    kal_uint32 left, cnt, event,mask;
#if defined(__ICUSB_SUPPORT__)
    kal_uint32 shiftIndex;
#endif    
    ...
    CCCI_UART_DBG_TRACE(MOD_TST, "[CCCI UART][Port %d] Request to write, length: %d", port, Length);
    ...
    do
    {
        ...
        if (left > cnt)
        {
             CCCI_UART_DBG_TRACE(MOD_TST, "[CCCI UART][Port %d] Write %d bytes (full)", port, cnt);
...
#if defined(__ICUSB_SUPPORT__)
            shiftIndex = left;
            CCCI_UART_DBG_TRACE(MOD_TST, "[CCCI UART][Port %d] Write %d bytes, src addr: 0x%X (partial)", port, left, Buffaddr);
...
DCL_STATUS CCCI_UART_Handler(DCL_DEV dev,DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
    ...
    CCCI_UART_DBG_TRACE(MOD_TST, "[CCCI UART] HDR entry, dev: %d, cmd: %d", dev, cmd);

#if defined(__ICUSB_SUPPORT__)
    //- check dev is ccci UART port
    if (!((dev >= uart_port_tst_ccci && dev <= uart_port_gps_ccci) || uart_port_sim_ccci == dev))
        return STATUS_INVALID_DEVICE;

    //- Remap sim uart port to next of gps port to make all ccci uart ports are continuous.
    //- After remapping, sim uart port will have the same uart port index with start_of_virtual_port in dcl.h.
    //- This is ok because start_of_virtual_port will not be dev that use CCCI uart.
    if (uart_port_sim_ccci == dev)
        dev = uart_port_gps_ccci + 1;    //- uart_port_gps_ccci: last port in continuous ccci uart ports.
#endif
