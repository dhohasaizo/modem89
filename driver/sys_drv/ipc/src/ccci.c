...
__attribute__  ((zero_init, section ("CACHED_EXTSRAM_ZI")))	kal_uint32     ccci_trans_log_index[CCCI_MAX_CHANNEL] = {};
__attribute__  ((zero_init, section ("CACHED_EXTSRAM_ZI")))	kal_uint32     ccci_need_check_owner_mask[2] = {};
__attribute__  ((zero_init, section ("CACHED_EXTSRAM_ZI")))	CCCI_TRANS_LOG ccci_trans_log_pool[CCCI_MAX_CHANNEL][DebugEntryNo]= {};
__attribute__  ((zero_init, section ("CACHED_EXTSRAM_ZI")))	CCCI_BUFF_T ccif_last_rx_snapshot[CCIF_MAX_PHY]= {};
__attribute__  ((zero_init, section ("CACHED_EXTSRAM_ZI")))	CCCI_BUFF_T ccif_last_tx_snapshot[CCIF_MAX_PHY]= {};
...
static void ccci_need_check_owner_mask_init(void)
{
    ...
    for (channel=0, i=0; channel<CCCI_MAX_CHANNEL; channel++, i++)
    {
        ...
        else if (CCCI_SIM_CHANNEL_TX <= channel &&  CCCI_SIM_ACK_CHANNEL_RX >= channel)
        {
            continue;
        } 
#endif
...
void ccci_init_handshake_phase1(void)
{
    ...
    *CCIF_ACK = 0xFFFFFFFF;
    ...
    for (index = 0; index < ap_md_share_data.UartPortNum; index ++)
    {
        ...
        if (0 == ap_md_share_data.UartShareMemBase[index])	break;
...
/*************************************************************************
* FUNCTION
*  kal_int32 ccci_fill_idle_task_state
*
* DESCRIPTION
*  This function is used to log idle task state.
*
* PARAMETERS
*  log_type    - Log type
...
*
*************************************************************************/
kal_int32 ccci_fill_idle_task_state(kal_uint32 log_type, kal_uint32 state_info)
{
    kal_uint32          *write_ptr;
    volatile kal_uint32 dummy_read;
    
    if( log_type > CCCI_IDLE_TASK_INFO_END)
        return KAL_FALSE;
        
    write_ptr = (kal_uint32 *)ap_md_share_data.ExceShareMemBase + log_type;
    
    *write_ptr = state_info;
    
    //- keep last log index;
    *((kal_uint32 *)ap_md_share_data.ExceShareMemBase + CCCI_IDLE_TASK_INFO_END) = (0x80000000 | log_type);
    
    //- read to device type memory to ensure data is drained to EMI.
    dummy_read = *write_ptr;

    if (dummy_read != state_info)
        return KAL_FALSE;
...
#if defined(__ICUSB_SUPPORT__)
    uart_handle =  DclSerialPort_Open(uart_port_sim_ccci, 0);
    DclSerialPort_RegisterCallback(uart_handle, &CCCI_Uart_Drv_Handler);
#endif
}
