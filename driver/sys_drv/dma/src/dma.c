...
VOID_FUNC DMA2_LISR_FUNC[DMA2_MAX_CHANNEL] =
{
   DMA_ERROR_LISR,
   DMA_ERROR_LISR,
   DMA_ERROR_LISR,
...
#if defined(__DMA2_SUPPORT__) //need to confirm 0824
...
#if !defined(__DMA_V3__)
SSDVT_DMA_INT_ZI_ATTR static DMA_GBL_CLASS dma_gbl_class_g;
SSDVT_DMA_INT_ZI_ATTR static const kal_uint8 dma_gbl_class_to_grade[DMA_MAX_GBL_CLASS + 1]
...
SSDVT_DMA_INT_ZI_ATTR static const DMA_GBL_PRIORITY_BANK dma_gbl_priority_bank[DMA_GBL_TOTAL_PRIORITY_BANK]
...
SSDVT_DMA_INT_RW_ATTR kal_uint32 DMA_Channel_Status = DMA_CHANNEL_MASK;
#else
SSDVT_DMA_INT_RW_ATTR kal_uint32 DMA_Channel_Status = 0xffff;
...
SSDVT_DMA_INT_RW_ATTR DMA_Master DMA_Owner[DMA_ALL_CHANNEL];
...
static DMA_SAVE_PARAM DMA_Save_Param[DMA_ALL_CHANNEL] __SECTION__(INTSRAM_ZI) __ZI__ ;
#if defined(__DMA2_SUPPORT__)
#if defined(MT6589) && defined(__MD2__)
static DMA_SAVE_PARAM DMA2_Save_Param[DMA2_ALL_CHANNEL] __ZI__ ;
#else
static DMA_SAVE_PARAM DMA2_Save_Param[DMA2_ALL_CHANNEL] __SECTION__(INTSRAM_ZI) __ZI__ ;
#endif
#endif //__DMA2_SUPPORT__
...
#if defined(__DMA_V3__)
//#pragma arm section rwdata = "INTSRAM_RW" , rodata = "INTSRAM_RODATA" , zidata = "INTSRAM_ZI"
static DMA_SAVE_PARAM DMA_Save_Param[DMA_MAX_CHANNEL] __SECTION__(INTSRAM_ZI) __ZI__ ; 
//#pragma arm section rwdata , rodata , zidata
#endif
...
static void DMA2_Quick_Config(kal_uint32 dma_no, kal_uint32 dst) __SECTION__(INTERNCODE);
...
void DMA2_HISR(void)
{
   ...
   for(index = 0; index < DMA2_MAX_CHANNEL; index++)
    ...
    #if defined(__DMA_V3__)
       if( DRV_Reg32(DMA2_GLBST_INT) & ( 1<<index_hw ) )
    #else //__DMA_V3__
...
void DMA2_LISR(void)
{
   ...
   for (
        index = 0; index < DMA2_MAX_CHANNEL; index++
            )
   {
	   kal_uint32 index_hw;
   ...
   #if !defined(__FUE__)   
	  for(index=DMA_VFIFO_CH_S; index<=DMA2_VFIFO_CH_E; index++)
	  {
		   kal_uint32 index_hw;
	   #if defined(__MULTI_CORE_SHARED_DMA__)	   
	   #if defined(DMA2_VFF_CH_NUM) && (DMA2_VFF_CH_NUM != 0)
...
SSDVT_DMA_INT_CODE_ATTR kal_uint8 DMA_FullSize_GetChannel(DMA_Master DMA_CODE)   /* Hw handler. handle = 1~3 */
{
   kal_uint8 index;
   kal_uint8 handle=0xff;
   kal_uint32 savedMask;
...
SSDVT_DMA_INT_CODE_ATTR void DMA_Config_Internal(kal_uint8 dma_no, DMA_INPUT *dma_menu, kal_bool fullsize, kal_bool b2w, kal_uint8 limit, kal_bool start)
{
   ...
   if(dma_menu->type!=DMA_HWTX_RINGBUFF && dma_menu->type!=DMA_HWRX_RINGBUFF && dma_menu->type!=DMA_FULLSIZE_TXRINGBUFF && dma_menu->type!=DMA_FULLSIZE_RXRINGBUFF)
   {
   ...
   {  
	   DMA_Save_Param[dma_no-1].len1 = (kal_uint32)*wppt<<(dma_menu->size);
	   DMA_Save_Param[dma_no-1].len2 = ((kal_uint32)dma_menu->count-(kal_uint32)(*wppt))<<(dma_menu->size);
	   DMA_Save_Param[dma_no-1].size = dma_menu->size;
   }
   ...
   if (dma_gbl_class == 0) {
      ...
      priority = kal_get_current_task_priority();
...
//#pragma arm section code = "INTERNCODE"
void DMA_Quick_Config(kal_uint32 dma_no, kal_uint32 dst) __SECTION__(INTERNCODE);
...
int DMA_Vfifo_SetChan(UART_PORT port)
{
...
#else //#if defined(__DMA_UART_VFIFO_SINGLE_TUNNEL__)
 {
	...
	if(port == uart_port1)
	{
	   ...
	   dma_con = (DMA_CON_MASTER_UART1RX << 20)|DMA_CON_DIR|DMA_CON_DRQ|DMA_CON_ITEN;
	   //DRV_WriteReg32(DMA_CON(VDMA_UART1RX_CH),dma_con);
	   DRV_WriteReg32(DMA_CON(UARTPort[port].Rx_DMA_Ch),dma_con);
	   dma_con = 0;
	   dma_con = (DMA_CON_MASTER_UART1TX << 20)|DMA_CON_DRQ;
	   //DRV_WriteReg32(DMA_CON(VDMA_UART1TX_CH),dma_con);
	   DRV_WriteReg32(DMA_CON(UARTPort[port].Tx_DMA_Ch),dma_con);
#endif //__DMA_V3__	   
 }
 else if(port == uart_port2)
 {
