...
#if defined (__MD1__) || defined (__MD2__)
      ipcStatus = IPC_RPC_EINT_GetAttribute(query, 24, SIM_HOT_PLUG_EINT_NUMBER, (void *)&iccHotPlug.eintNo, 4);
#else
...
#if defined (__MD1__) || defined (__MD2__)
      ipcStatus = IPC_RPC_EINT_GetAttribute(query, 24, SIM_HOT_PLUG_EINT_DEBOUNCETIME, (void *)&iccHotPlug.debounceTime, 4);
#else
...
#if defined (__MD1__) || defined (__MD2__)
      ipcStatus = IPC_RPC_EINT_GetAttribute(query, 24, SIM_HOT_PLUG_EINT_POLARITY, (void *)&iccHotPlug.polarity, 4);
#else
...
#if defined (__MD1__) || defined (__MD2__)
      ipcStatus = IPC_RPC_EINT_GetAttribute(query, 24, SIM_HOT_PLUG_EINT_SENSITIVITY, (void *)&iccHotPlug.sensitivity, 4);
#else
...
#if defined (__MD1__) || defined (__MD2__)
      ipcStatus = IPC_RPC_EINT_GetAttribute(query, 24, SIM_HOT_PLUG_EINT_SOCKETTYPE, (void *)&iccHotPlug.socketType, 4);
#else
...
LEAVE_REG_EINT:
	   kal_sprintf(sim_dbg_str,"[SIM_DRV]EINT: %d %d %d %d %d %d", ipcStatus, iccHotPlug.eintNo, iccHotPlug.debounceTime, iccHotPlug.polarity, iccHotPlug.sensitivity, iccHotPlug.socketType);
#if	!defined( __MAUI_BASIC__)
         tst_sys_trace(sim_dbg_str);
#else
	      dbg_print(sim_dbg_str);
