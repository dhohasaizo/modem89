...
static void usim_deactivation(sim_HW_cb *hw_cb)
{
	...
	kal_sleep_task(16);
	   ...
	   {
			   DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC20, usim_dcb->ev_status,SIM_Reg(SIM0_BASE_ADDR_MTK + 0x70),SIM_Reg(SIM0_BASE_ADDR_MTK + 0x74),hw_cb->simInterface,SIM_Reg(SIM0_BASE_ADDR_MTK + 0x00));
		 }
//	   else
//	   	ASSERT(0);
	}
		...
		{ // receiving a S-block of request
			...
			switch(pcb)
			{
				...
				case ABORT_REQ:
					...
					usim_dcb->rx_index = 0;
					DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC41, usim_dcb->tx_size, usim_dcb->tx_index, usim_dcb->rx_size,usim_dcb->rx_index,pcb);
