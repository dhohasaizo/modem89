...
volatile uint16* SALI_Bgsnd_GetBuf()
{
	volatile uint16* ptr;
	ptr = DSP_DM_ADDR(BGSND_PAGE_NUM, *DSP_SPH_BGS_ADDR);
	return ptr;
}

uint16 SALI_Bgsnd_GetDataLen()
{
	return *DSP_SPH_BGS_LEN;
}
...
void SALI_PcmEx_Config(Sal_PCMEx_Config_t* cfg)
{
	if(cfg->type == SAL_PCMEX_TYPE_PNW){
		...
		if(cfg->M2D_ul2)
			*DSP_SPH_PNW_CTRL_UL |= (0x0001 << 14);
		else
			*DSP_SPH_PNW_CTRL_UL &= ~(0x0001 << 14);
		if(cfg->D2M_ul2)
			*DSP_SPH_PNW_CTRL_UL |= (0x0001 << 13);
		else
			*DSP_SPH_PNW_CTRL_UL &= ~(0x0001 << 13);			
		if(cfg->afterEnh_ul2)
			*DSP_SPH_PNW_CTRL_UL |= (0x0001 << 12);
		else
			*DSP_SPH_PNW_CTRL_UL &= ~(0x0001 << 12);		
...
volatile uint16* SALI_PcmEx_GetBuf(Sal_PcmEx_BufId_t id)
{
	...
	switch(id){
		...
		case SAL_PCMEX_PNW_BUF_UL2:
			...
			break;
		case SAL_PCMEX_PNW_BUF_DL:
			addr = DSP_SPH_PNW_BUF_DL;
			break;		
...
uint16 SALI_PcmEx_GetBufLen(Sal_PcmEx_BufId_t id)
{
	...
	switch(id)
	{
		...
		case SAL_PCMEX_PNW_BUF_DL:
			...
			break;
		case SAL_PCMEX_BUF_SE:
			if(*DSP_SPH_BAND&0x2)//bit1 -> codec band, bit0 -> sch band
				len = 320;
			else
				len = 160;
			break;
		case SAL_PCMEX_BUF_SE2:
		case SAL_PCMEX_BUF_SD:
			if(*DSP_SPH_BAND&0x1)//bit1 -> codec band, bit0 -> sch band
				len = 320;
			else
				len = 160;
			break;
...
void SALI_AGC_Config(Sal_AGC_Config_t *cfg)
{
	uint16 value = 0;
	if(cfg->main_switch)
		value |= 0x8000;
