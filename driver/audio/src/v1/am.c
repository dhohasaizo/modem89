 ...
 *
 * 11 06 2013 lanus.chao
 * [MOLY00045306] Snoopy(S960) AudioPatch 申?
 * .
 *
 ...
 * .
 *
 * 04 29 2013 sheila.chen
 * [MOLY00020886] ???????????MIC?????
 * Provide sencond mic data via PCMNWAY API
 *
 ...
 * .
 *
 * 03 21 2013 sheila.chen
 * [MOLY00012352] [prada] ??????patch
 * Lenovo feature change (48MP)
 *
 * 02 19 2013 yt.liou
 * [MOLY00010340] DMNR modify for CMCC and DMNR calibration -- mcu part
 * .
 *
 * 01 30 2013 thomas.chen
 ...
 * 1. Fix bug that DL gain is fix under idle pcmNway
 * 2. Gain is apply too late
 *
 * 01 21 2013 sheila.chen
 * [MOLY00008440] [MT6589] Disable AGC under BT mode
 * Move AGC from hw to enhancement
 * initial local variable before using SAL functions
 *
 * 01 10 2013 sheila.chen
 ...
 * Speech Set Mode modification
 *
 * 12 17 2012 sheila.chen
 * [MOLY00000112] [MT6583] Pre-integration
 * Add DL mute debug
 *
...
void AM_SpeechOn(uint32 rat_mode, uint32 extraInfo)
{
   ...
   AM_Enqueue( AM_DSP_SpeechOn, rat_mode, extraInfo, (rat_mode == RAT_2G_MODE) );
...
void AM_PCM8K_RecordOn( AM_PCM8KREC_APP_TYPE appType )
{
   ...
   if(!(am.state & AM_STATE_SPEECH) && (AM_PCM8KREC_APP_TYPE_VMEPL != appType)) // idle record
#if defined(__BT_SUPPORT__)
      if(!AM_IsBTCordlessMode())
#endif
      DSP_DynamicDownload( DDID_NOISE_REDUCTION );
	  ...
	  case AM_PCMEX_TYPE_REC_PLAY_CAL: //4: record and play dmnr calibration	
         ...
         am.state |= AM_STATE_VOIP; 			
		   ...
		   SAL_ENH_SetULPath(true);   
			...
			cfg.D2M_ul2 = (0!=(pcmExData->cfgUl2 & USE_D2M_PATH));
			cfg.M2D_ul2 = (0!=(pcmExData->cfgUl2 & USE_M2D_PATH));
			cfg.afterEnh_ul2 = (0!=(pcmExData->cfgUl2 & DATA_SELECT_AFTER_ENH)); // shoud before
