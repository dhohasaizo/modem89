...
*/
	...
	// get current buffer data length. (16k sampling rate)
	count = bgsndGetDataCount();
	
	//set up the DSP address and pointers
    dsp_buf_len = SAL_Bgsnd_GetDataLen();
	
	if(dsp_buf_len == 320)
		isNb = false;
	else if(dsp_buf_len == 160){
		isNb = true;
		count >>= 1;
		}
	else
	...
	toPtr = SAL_Bgsnd_GetBuf();
	
	// kal_prompt_trace(MOD_L1SP, "[pDebug] dsp_buf_addr(toPtr): %x, dsp_buf_len: %d word", toPtr, dsp_buf_len);
	// kal_prompt_trace(MOD_L1SP, "[pDebug] count: %d word", count);
	kal_dev_trace(TRACE_INFO, L1SND_DSP_BUFFER_INFO, count, toPtr, dsp_buf_len);
	
   if (count > dsp_buf_len)
      count = dsp_buf_len;
...
/* under construction !*/
	   ...
	   if (segment > 0) {
	      ...
	      if (bgSnd.bufRead >= bgSnd.bufSize)
			...
			// kal_prompt_trace(MOD_L1SP, "[pDebug]bgsnd_WriteToDSP1, bgSnd.bufRead=%d, bgSnd.bufWrite=%d", bgSnd.bufRead, bgSnd.bufWrite);
	   ...
	   if (segment > 0) {
	      ...
	      if (bgSnd.bufRead >= bgSnd.bufSize)
	         bgSnd.bufRead = (bgSnd.bufRead-bgSnd.bufSize);
			// kal_prompt_trace(MOD_L1SP, "[pDebug]bgsnd_WriteToDSP2, bgSnd.bufRead=%d, bgSnd.bufWrite=%d", bgSnd.bufRead, bgSnd.bufWrite);
	...
	kal_uint8 ULGainLevel , kal_uint8 DLGainLevel)
{
	...
	L1Audio_HookHisrHandler(D2C_SOUND_EFFECT_INT_ID, bgsndHisr, 0);  
...
void BGSND_Close(void)
{
	...
	// L1Audio_UnRegister_Handle(MEDIA_BGSND);


	//L1Audio_FreeAudioID( bgSnd.aud_id );

	// bgSnd.isRunning = KAL_FALSE;
