...
volatile uint16* SAL_Bgsnd_GetBuf()
{
	return SALI_Bgsnd_GetBuf();
}

uint32 SAL_Bgsnd_GetDataLen()
{
	return SALI_Bgsnd_GetDataLen();
}
...
void SAL_AGC_Config(Sal_AGC_Config_t *cfg)
{
	ASSERT_REBOOT(cfg != 0);
	SALI_AGC_Config(cfg);
}