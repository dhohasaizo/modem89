...
sub gen_hq_only_cmm_from_template_cmm
{
    ...
    $strReplacedContent =~ s/\[ELFPARA\]//g;
    ...
    my $strBeforeDump = &get_version_from_elf_template();
    my $strDumpRegions = &gen_dump_regions_template();
    my $strAfterDump = &get_version_from_memory_dump_template() . &version_compare_template();
    my $strBinary = $strBeforeDump . $strDumpRegions . $strAfterDump;
    $strReplacedContent =~ s/\[BINARY\]/$strBinary/;
    
    ## Replace [CHECK_CODE_INTEGRITY] with DIFF code
    my $strCHECK_CODE_INTEGRITY = &gen_check_code_integrity_template();
    $strReplacedContent =~ s/\[CHECK_CODE_INTEGRITY\]/$strCHECK_CODE_INTEGRITY/;
...
sub get_version_from_memory_dump_template
{
    my $template;
    $template = <<"__TEMPLATE";
...
sub gen_dump_regions_template
{
    ...
    {
        next if($PLATFORM eq "MT6280" and $strGroupName =~ /^ROM/); # for mt6280 finding emi issue
...
sub gen_check_code_integrity_template
{
    return "" if($PLATFORM ne "MT6280");
    &LISInfo::ClassifyRegionType(\%MAKEFILE_OPTIONS);
    my $DumpRegions_ref = &LISInfo::GetDumpExeRegions(\%MAKEFILE_OPTIONS);
    my $BIN_PATH = $BUILDDIR ."\\". $MAUI_BIN;
    print "-----------------DUMP CODE INTEGRITY Regions-----------------\n";
    my $strBaseForCodeCheck = undef;
    foreach my $strGroupName (%$DumpRegions_ref)
    {
        next if($strGroupName !~ /^ROM/);
        my $array_ref = $DumpRegions_ref->{$strGroupName};
        foreach my $RegionItem (@$array_ref)
        {
            my $RegionName = $RegionItem->[0];
            my $strBase = &LISInfo::GetDumpRegionBase($strGroupName, $RegionName, $BIN_PATH, $PLATFORM);
            $strBaseForCodeCheck = $strBase if($strGroupName =~ /^ROM/); # for mt6280 finding emi issue
            print "[$strGroupName]$RegionName: $strBaseForCodeCheck\n";
            last if(defined $strBaseForCodeCheck);
        }
    }
    print "----------------------------------------------\n";
    my $template = <<"__TEMPLATE";
;check code integrity
D.LOAD.BINARY &BIN_PATH\\sys_mem_$strBaseForCodeCheck.bin $strBaseForCodeCheck /byte /nosymbol /DIFF
IF FOUND()
(
    D.LOAD.BINARY &BIN_PATH\\sys_mem_$strBaseForCodeCheck.bin $strBaseForCodeCheck /byte /nosymbol
    GOSUB WARNNING_CODE_DAMAGE
)
__TEMPLATE
    return $template;
}
