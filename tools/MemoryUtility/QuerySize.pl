...
my ($ROSize, $RWSize, $ZISize, $RWZISize);
...
elsif($file =~ /\.map$/)
{
	if($query_name =~ /\.o/)
	{
		$ROSize = &LinkerOutputParser::ListObjSummaryInfo($query_name, "RO Data");
		$RWZISize = &LinkerOutputParser::ListObjSummaryInfo($query_name, "RWZI Data");
	}
	elsif(($query_name =~ /\.l/) || ($query_name =~ /\.a/))
	{
		$ROSize = &LinkerOutputParser::ListLibSummaryInfo($query_name, "RO Data");
		$RWZISize = &LinkerOutputParser::ListLibSummaryInfo($query_name, "RWZI Data");
	}
	else
	{
		&Usage();
