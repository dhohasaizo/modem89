...
sub GetChipInsideRAMsize
{
    ...
    &GetLdsInfo($LDSFILE);
    my $MemoryInfo_ref = $g_MAUIlds->GetMEMROYInfo();
...
#              $RAMregion_href = a Hash reference to store RAM regions
...
sub GetRAMregion
{
    my ($BB_path, $LDSFILE, $RAMregion_href, $MAKEFILE_ref) = (@_);
    my $CUSTOM_FEATURECONFIG_H = $BB_path . '/' . "custom_FeatureConfig.h";
    my $nRAMnum = 0;
    ...
    {
        #print $_."\n" if $g_debug == 1;
        next if($_ =~ /GFH$|SIGNATURE|EMIINIT_CODE|^INTSRAM|^L2TCM|DUMMY/);
        $nRAMnum++;
        $RAMregion_href -> {$_} = [];
    }
...
sub GetROMregion
{
    my ($BB_path, $LDSFILE, $ROMregion_href, $MAKEFILE_ref) = (@_);
    my $nROMnum = 0;
...
#****************************************************************************
# Subroutine:  GetLdsInfoByScatInfo - to query Lds info by ScatInfo.pm
# Parameters:  $LDSFILE = lds file
#****************************************************************************
sub GetLdsInfoByScatInfo
{
    my ($LDSFILE) = (@_);
...
{
    my ($LDSFILE) = (@_);
    $g_MAUIlds = new ldsInfo;
    $g_MAUIlds->ParseLinkerScript($LDSFILE);
}
