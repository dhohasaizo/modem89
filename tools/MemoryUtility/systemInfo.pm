...
sub GetRAMregion
{
    my ($BB_PATH, $strFilePath, $RAMregion_href, $MAKEFILE_ref) = (@_);
    print "file path = $strFilePath\n" if($DebugPrint == 1);
    my $strCompiler = &FileInfo::GetCompiler($MAKEFILE_ref);
    ...
    if($strCompiler eq "RVCT")
    {
        $nRAMnum = &sysInfoProcScat::GetRAMregion($BB_PATH, $strFilePath, $RAMregion_href, $MAKEFILE_ref);
    }
    elsif($strCompiler eq "GCC")
    {
        $nRAMnum = &sysInfoProcLds::GetRAMregion($BB_PATH, $strFilePath, $RAMregion_href, $MAKEFILE_ref);  
    }
    return ($nRAMnum);
}
