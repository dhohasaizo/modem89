...
sub ldsGen_verno
{
    return " m0.22 + ldsFrame.pm ".&ldsFrame::ldsFrame_verno;
...
sub CollectMemorySetting  #CallBack func
{
    ...
    no strict 'refs';
    my $Setting_href = &{$func};
    my $nReservedSize = &GetReservedSize_FromBottomToTop_OnRAM(undef, $RegionList_ref, $Index_ref,
                                           "DSP_RX", "DUMMY_END");
    $Setting_href->{"RESERVED_FOR_DUMMY_END"} = &CommonUtil::Dec2Hex($nReservedSize);
    return $Setting_href;
}
...
sub GenSTART_RAM # Fill in RegionList.csv
{
    my ($MEMORYSetting_href, $RegionList_aref, $Index_ref) = @_;
    return $MEMORYSetting_href->{RAM}->[0];
}
...
sub GetAddress_FromBottomToTop_OnRAM
{
    ...
    foreach my $item (@$RegionList_aref)
    {
        ...
        $nBase = $strTopRegionName =~ /cached/i ? (0xf0000000|$nBase) : $nBase;
...
sub MT6280_MemorySetting  # Called by CollectMemorySetting()
{
    ...
    my %Setting = (RAM_BASE => &CommonUtil::Dec2Hex(0x0),
                   RAM_LEN  => &CommonUtil::Dec2Hex($nRamSize),
                   VRAM_BASE =>&CommonUtil::Dec2Hex(0xf0000000 | 0x0),
                   );
    return \%Setting;
}
sub MT6575_MemorySetting  # Called by CollectMemorySetting()
{
    ...
    my %Setting = (ROM_BASE => &CommonUtil::Dec2Hex($fbase),
                   ROM_LEN  => &CommonUtil::Dec2Hex($flength),
                   RAM_BASE => &CommonUtil::Dec2Hex($flength),
                   RAM_LEN  => &CommonUtil::Dec2Hex($rs),
                   VRAM_BASE =>&CommonUtil::Dec2Hex(0xf0000000 | $flength),
                   );
    return \%Setting;
}
...
{#smart phone doesn't use $g_nRamSize
    my ($nRamSize, $nRWZIBase) = (0, 0); # RamSize means MD available space
    print "mode=".&FileInfo::get("MODE")."\n";
    if(&FileInfo::find("MODE", "GPRS"))
    {
        $nRamSize = ($g_bb eq "MT6589_MD1") ?  0xA00000 : 0x800000; #MT6589_MD1_GPRS(10MB), MT6589_MD2_GPRS(8MB)
        $nRWZIBase = ($g_bb eq "MT6589_MD1") ?  0x400000 : 0x400000; #MT6589_MD1_GPRS(4MB), MT6589_MD2_GPRS(4MB)
    }
    else
    {
        $nRamSize = ($g_bb eq "MT6589_MD1") ?  0x1600000 : 0x1500000; #MT6589_MD1_HSPA(22MB), MT6589_MD2_TDD128HSPA(21MB)
        $nRWZIBase = ($g_bb eq "MT6589_MD1") ?  0x700000 : 0x600000; #MT6589_MD1_HSPA(7MB), MT6589_MD2_TDD128HSPA(6MB)
    }
    &sysUtil::sysgen_die("RamSize($nRamSize) can't be 0 in MT6589_MemorySetting()!", __FILE__, __LINE__) 
        if(0==$nRamSize);
    
    my $nLeftRam = $nRamSize - $nRWZIBase;
    my %Setting = (ROM_LEN  => &CommonUtil::Dec2Hex($nRWZIBase),
                   RAM_BASE => &CommonUtil::Dec2Hex($nRWZIBase),
                   RAM_LEN  => &CommonUtil::Dec2Hex($nLeftRam),
                   VRAM_BASE =>&CommonUtil::Dec2Hex(0xf0000000 | $nRWZIBase),
                   );
    return \%Setting;
}
