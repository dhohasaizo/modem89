...
#*************************************************************************************************
if($LEVEL == 1) {
  ($LINKLOG,$ckImgSize,$MAKEFILE,$ckSYSDRV_LOG,$ckBBU_LOG,$SYS_PATH,$FLASH_CFG,$FSUSAGE_LOG,$NVRAMID_LOG,$FEATURE_OVER,$SYSDRV,$NVRAM_PSEUDO,$PROJECT_NAME,$tmp_fld) = @ARGV;
  my $LINK_ERR = 0;
  my $DP_LINK_ERR = 0;
  my $AAPMC_LOG = "$tmp_fld\\AAPMC.log";
  ...
  #    -- it will be removed in backup action in build.mak
  # >> intermed. file pattern: xxx_LinkError.tmp
  if($LINKLOG =~ /(.+)\\(.+)/) {
    $TARGDIR = $1;
    $logname = $2;
  }
  system("del /q /f $tmp_fld\\~VIVA_LinkError.tmp > nul")if(-e "$tmp_fld\\~VIVA_LinkError.tmp");
  system("del /q /f $tmp_fld\\~DP_LinkError.tmp > nul")if(-e "$tmp_fld\\~DP_LinkError.tmp");
  system("del /q /f $tmp_fld\\~FS_LinkError.tmp > nul")if(-e "$tmp_fld\\~FS_LinkError.tmp");
  system("del /q /f $tmp_fld\\~FS_LinkError_BBU.tmp > nul")if(-e "$tmp_fld\\~FS_LinkError_BBU.tmp");
    ...
    if(((defined $nand_flash_booting) && ($nand_flash_booting eq "ONDEMAND"))||((defined $emmc_booting) && ($emmc_booting eq "ONDEMAND"))) {
  	...
  	if($DP_LINK_ERR) {
       ...
       if($SYSDRV_ERR && -e $ckBBU_LOG){
         ...
         $LINK_ERR = 1;
         system("copy /y $tmp_fld\\~FS_LinkError.tmp $tmp_fld\\~FS_LinkError_BBU.tmp  > nul");
  ...
  if(-e "$tmp_fld\\~FS_LinkError.tmp" && -e "$tmp_fld\\~VIVA_LinkError.tmp"){
    system("copy /y $tmp_fld\\~FS_LinkError.tmp $tmp_fld\\~FS_LinkError.ori.tmp > nul");
    print "perl tools\\cksysdrv.pl $FLASH_CFG $FSUSAGE_LOG $NVRAMID_LOG $FEATURE_OVER $SYSDRV $NVRAM_PSEUDO $SYS_PATH $tmp_fld\\~FS_LinkError.tmp $PROJECT_NAME AAPMC_temp.log> $LogFolder\\FileSystemConfig.log 2>&1\n" if($DEBUG);
    $out_str = "Calculating correct File System value...";
    $result = system("perl tools\\cksysdrv.pl $FLASH_CFG $FSUSAGE_LOG $NVRAMID_LOG $FEATURE_OVER $SYSDRV $NVRAM_PSEUDO $SYS_PATH $tmp_fld\\~FS_LinkError.tmp $PROJECT_NAME AAPMC_temp.log >> $LogFolder\\FileSystemConfig.log 2>&1") ;
    system("move /Y $tmp_fld\\~FS_LinkError.ori.tmp $tmp_fld\\~FS_LinkError.tmp > nul");
    system("del /q /f AAPMC_temp.log > nul")if(-e "AAPMC_temp.log");
    if(($result>>8) == ERR::CANNOT_ADJUST or ($result>>8) == ERR::NO_MODIFY) {
  ...
  my ($LIS_FILE,$SYS_PATH,$MAKEFILE,$LogFolder,$BIN_FOLDER,$FLASH_CFG,$PROJECT_NAME,$FSUSAGE_LOG,$NVRAMID_LOG,$FEATURE_OVER,$SYSDRV,$NVRAM_PSEUDO,$tmp_fld) = @ARGV;
  ...
  if(-e "$tmp_fld\\~VIVA_LinkError.tmp") {
    &error_handler("File: $LIS_FILE did NOT exist!", __FILE__, __LINE__) if(!-e $LIS_FILE);
    ...
    print "perl tools\\vivaConfig.pl $LIS_FILE $BIN_FOLDER $SYS_PATH $MAKEFILE $FLASH_CFG $tmp_fld\\~VIVA_LinkError.tmp $AAPMC_LOG> $LogFolder\\vivaConfig.log 2>&1\n" if($DEBUG);
    $out_str = "Calculating correct VIVA memory value...";
    $result = system("perl tools\\vivaConfig.pl $LIS_FILE $BIN_FOLDER $SYS_PATH $MAKEFILE $FLASH_CFG $tmp_fld\\~VIVA_LinkError.tmp $AAPMC_LOG> $LogFolder\\vivaConfig.log 2>&1") ;
    $remd_str = &getRecomdValue("$tmp_fld\\~VIVA_LinkError.tmp");
    $check_scrip = "vivaConfig";
  } elsif (-e "$tmp_fld\\~DP_LinkError.tmp") {
    &error_handler("File: $LIS_FILE did NOT exist!", __FILE__, __LINE__) if (!-e $LIS_FILE);
    #re-calculate MAX BIN size for DP and MBA
    print "perl tools\\DemandPagingConfig.pl $LIS_FILE $SYS_PATH $MAKEFILE $tmp_fld\\~DP_LinkError.tmp $PROJECT_NAME $AAPMC_LOG> $LogFolder\\DemandPagingConfig.log 2>&1\n" if($DEBUG);
    $out_str = "Calculating correct MAX BIN SIZE value...";
    $result = system("perl tools\\DemandPagingConfig.pl $LIS_FILE $SYS_PATH $MAKEFILE $tmp_fld\\~DP_LinkError.tmp $PROJECT_NAME $AAPMC_LOG> $LogFolder\\DemandPagingConfig.log 2>&1") ;
    $remd_str = &getRecomdValue("$tmp_fld\\~DP_LinkError.tmp");
    $check_scrip = "DemandPagingConfig";
  } elsif (-e "$tmp_fld\\~FS_LinkError.tmp") {
    #re-calculate FS size
    print "perl tools\\cksysdrv.pl $FLASH_CFG $FSUSAGE_LOG $NVRAMID_LOG $FEATURE_OVER $SYSDRV $NVRAM_PSEUDO $SYS_PATH $tmp_fld\\~FS_LinkError.tmp $PROJECT_NAME $AAPMC_LOG> $LogFolder\\FileSystemConfig.log 2>&1\n" if($DEBUG);
    $out_str = "Calculating correct File System value...";
    $result = system("perl tools\\cksysdrv.pl $FLASH_CFG $FSUSAGE_LOG $NVRAMID_LOG $FEATURE_OVER $SYSDRV $NVRAM_PSEUDO $SYS_PATH $tmp_fld\\~FS_LinkError.tmp $PROJECT_NAME $AAPMC_LOG >> $LogFolder\\FileSystemConfig.log 2>&1") ;
    $remd_str = &getRecomdValue("$tmp_fld\\~FS_LinkError.tmp");
  ...
  if((($result>>8) == ERR::AAPMCLOG_SUCCESS && $Status != ERR::ERR_MODIFYFAIL) or
     (($result>>8) == ERR::AUTOCONFIG_SELF_MODIFY_SUCCESS)) {
  	print "$out_str\n";
  	print STDERR "$out_str\n";
  } else {
  	if($Status != 0) {
  		$error_code = "Error[".($result>>8)."-".$Status."]";
  	} else {
  		$error_code = "Error[".($result>>8)."]";
  	}
  	print "$error_code: $out_str\n";
  	die "$error_code: $out_str";
  }
...
sub ckVivaError{
  ...
  if($CK_VIVA_ERR) {
    open(TEMPFILE,">$tmp_fld\\~VIVA_LinkError.tmp") or &error_handler("cannot open $tmp_fld\\~VIVA_LinkError.tmp!", __FILE__, __LINE__);
    print TEMPFILE ".";
    close TEMPFILE;
  }
  return $CK_VIVA_ERR;
...
sub ckDemandPagingError{
  ...
  if($CK_DP_ERR) {
  	open(TEMPFILE,">$tmp_fld\\~DP_LinkError.tmp") or &error_handler("cannot open $tmp_fld\\~DP_LinkError.tmp!", __FILE__, __LINE__);
...
sub cksysdrvError{
  ...
  if($CK_SYSDRV_ERR) {
  	open(TEMPFILE,">$tmp_fld\\~FS_LinkError.tmp") or &error_handler("cannot open $tmp_fld\\~FS_LinkError.tmp!", __FILE__, __LINE__);
