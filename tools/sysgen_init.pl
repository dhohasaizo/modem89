...
rmtree($output_folder) or &error_handler("delete $output_folder folder failed!", __FILE__, __LINE__) if(-d $output_folder);	
...
foreach (@files) {
  ...
  if(&AAPMC_COPYER::IsNeededToCopy($output_folder,$input_folder,$replaced_filename)) {
    print "Need to Copy\n";
    ...
    $backupfile = "$output_folder/$replaced_filename\.$extension";
    if(-e $backupfile) {
      print "Delete $backupfile\n" if($Debug);
      unlink $backupfile or &error_handler("delete $backupfile file failed!", __FILE__, __LINE__);
    }
  }else {
    print "Skip\n";
  }
