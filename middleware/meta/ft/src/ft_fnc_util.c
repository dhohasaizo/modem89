...
{
    g_b_ft_assert_check_enable = req->cmd.m_rSetPwrOffParasReq.b_TargetAssertCheckFlag;
    if(req->cmd.m_rSetPwrOffParasReq.b_SetCurRecvMsgTimes == KAL_TRUE)
        g_u1_msg_recv_times = req->cmd.m_rSetPwrOffParasReq.m_u1CurRecvMsgTimes;
    cnf->status = FT_CNF_OK;
}
