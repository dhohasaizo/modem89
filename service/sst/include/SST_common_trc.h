 ...
 ****************************************************************************/
	...
	/* Phase 2: Analysis */
	TRC_MSG(SST_DUMP_STACK, "Active call path from current stack pointer (0x%X) as above.")
	TRC_MSG(SST_DUMP_PROBLEM_STACK, "Call path of problematic task %c%c%c%c%c%c%c%c as below.")
	TRC_MSG(SST_DUMP_STACK_FAILED, "No reasonable call path retreivable from current stack pointer!")
	...
	TRC_MSG(SST_PHASE3_DUMP_MEMORY, "Seriouos error, please dump memory and let system service member to give you a hand!")
